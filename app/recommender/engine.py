...
# ✅ FIXED: logging + safety
async def build_index(session: AsyncSession) -> dict:
    rows = await safe_execute_fetchall(session, text("SELECT * FROM jobs"))

    if not rows:
        logger.warning("No jobs found when building index")
    ...
    for r in rows:
        try:
            ...
            logger.exception("Error processing job row: %s", e)

    logger.info(f"Building index for {len(job_ids)} jobs")

    return await build_index_from_texts(job_texts, job_ids)
...
async def recommend_for_user(
    ...
    user_text = await build_user_profile(session, user_id)
    if not user_text:
        logger.info("Empty profile for user %s", user_id)
        return []
...
async def fetch_jobs_by_ids(session: AsyncSession, ids: List[int]):
    ...
    rows = await safe_execute_fetchall(
        session,
        text(f"SELECT * FROM jobs WHERE id IN ({', '.join(map(str, ids))})")
    )
